...
const adminlqApp = new Vue ({
    el: '#adminlq-app',
    data: {
        sidebar_hidden_state: window.innerWidth <= ADMINLQ_TABLET_MAX_WIDTH,
        ...
        adminlqBlankStyle: {
            'width': (window.innerWidth <= ADMINLQ_TABLET_MAX_WIDTH)?ADMINLQ_SIDEBAR_WIDTH_HIDDEN:ADMINLQ_SIDEBAR_WIDTH,
        },
        antIndentSvgIconClass: {
            'is-hidden': !(window.innerWidth <= ADMINLQ_TABLET_MAX_WIDTH)
        },
        ...
        sidebarStyle: {
            ...
            'transform': (window.innerWidth <= ADMINLQ_TABLET_MAX_WIDTH)?`translateX(-${ADMINLQ_SIDEBAR_WIDTH})`:'translateX(0px)'
        },
        maskForSidebarStyle: {
    ...
    methods: {
        topbarSidebarToggle: (event) => {
            if(window.innerWidth <= ADMINLQ_TABLET_MAX_WIDTH){
                ...
                if(adminlqApp.sidebar_accordion_state){
                    adminlqApp.adminlqBlankStyle['width'] = ADMINLQ_SIDEBAR_WIDTH;
                    adminlqApp.sidebarStyle['transform'] = 'translateX(0px)';
                    adminlqApp.antIndentSvgIconClass['is-hidden'] = true;
                    adminlqApp.antOutdentSvgIconClass['is-hidden'] = false;
                    ...
                    adminlqApp.adminlqBlankStyle['width'] = ADMINLQ_SIDEBAR_WIDTH_ACCORDION;
...
window.onresize = () => {
    if(window.innerWidth <= ADMINLQ_TABLET_MAX_WIDTH){
        adminlqApp.sidebarStyle['transform'] = `translateX(-${ADMINLQ_SIDEBAR_WIDTH})`;
        adminlqApp.adminlqBlankStyle['width'] = ADMINLQ_SIDEBAR_WIDTH_HIDDEN;
        adminlqApp.antOutdentSvgIconClass['is-hidden'] = true;
        adminlqApp.antIndentSvgIconClass['is-hidden'] = false;
        adminlqApp.sidebar_hidden_state = true;
    }else{
